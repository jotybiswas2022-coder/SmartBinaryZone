        ...
        <div class="d-flex gap-2 align-items-center">
            <div class="input-group" style="max-width:600px;">
                ...
                <input type="text" id="liveSearch" name="q" value="{{ $query ?? '' }}" 
                       ...
                       placeholder="Live search by title, category or content..."
            ...
            @if(request()->has('q') && request()->q != '')
                <a href="{{ route('admin.blog.index') }}" class="btn btn-outline-secondary rounded-3" style="border-color:#e2e8f0;">
        ...
        <div class="mt-2" id="searchInfo">
            @if($query ?? false)
                <small class="text-muted">
                    ...
                    <span id="resultCount">{{ $posts->count() }}</span> post(s) found
                </small>
            @endif
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" style="min-width:800px;">
                    ...
                    <tbody id="blogTableBody">
                        @include('backend.blog._table_rows', ['posts' => $posts])
                    </tbody>
                </table>
            </div>
        </div>
...
(function() {
    ...
    bindBlogEvents();
    window.bindBlogEvents = bindBlogEvents;
})();
...
(function() {
    ...
    function performSearch(query) {
        ...
        var url = '{{ route('admin.blog.index') }}' + '?q=' + encodeURIComponent(query);
        ...
        .then(function(data) {
            ...
            if (countBadge) {
                countBadge.innerHTML = '<i class="bi bi-database me-1"></i> ' + data.count + ' Posts';
            }

            if (searchInfo) {
                if (query) {
                    searchInfo.innerHTML = '<small class="text-muted"><i class="bi bi-info-circle me-1"></i>Showing results for "<strong>' + escapeHtml(query) + '</strong>" — ' + data.count + ' post(s) found</small>';
                } else {
                    searchInfo.innerHTML = '';
                }
            }

            if (window.bindBlogEvents) window.bindBlogEvents();
        })
